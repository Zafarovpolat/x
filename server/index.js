    ...
    ws.on('message', async message => {
        ...
        try {
            ...
            if (parsedMessage.answer && clients.get(clientId).role === 'exam') {
                ...
                // Отправляем ответ всем клиентам exam для немедленного отображения
                wss.clients.forEach(client => {
                    if (client.readyState === WebSocket.OPEN && clients.get(client.clientId).role === 'exam') {
            ...
            if (parsedMessage.type === 'timerUpdate' && clients.get(clientId).role === 'helper') {
                console.log('Получено timerUpdate от helper:', parsedMessage);
                if (!activeExams.has(clientId)) {
                    console.log('Создание нового экзамена для clientId:', clientId);
                    activeExams.set(clientId, { userInfo: '', questions: [], timer: parsedMessage.timer });
                }
                activeExams.get(clientId).timer = parsedMessage.timer;
                wss.clients.forEach(client => {
                    if (client.readyState === WebSocket.OPEN && clients.get(client.clientId).role === 'exam') {
                        console.log('Отправка timerUpdate клиенту exam:', client.clientId);
                        client.send(JSON.stringify({
                            type: 'timerUpdate',
                            clientId: clientId,
